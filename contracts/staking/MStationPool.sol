...
// OwnableUpgradeable,
contract MStationPool is Ownable, ReentrancyGuard, ERC20 {
    using SafeMath for uint256;

    // The address of the smart chef factory
    address public MSTATION_CASTLE_FACTORY;
    ...
    struct UserInfo {
        ...
        uint256 nftSlot; // slot reward nft from 1 -> total nft reward.
        uint256 nftStakingBlock; // block start stake to earn NFT.
    }

    // nft address for reward.
    address public nftAddress;
    // list of NFT token id for reward.
    uint256[] nftIdRewards;
    // counter of slot nft taken by staker
    uint256 nftSlotTaken;
    // min number of token staked for earn NFT
    uint256 public minStakeForNFT;
    // queue staker ready for reward.
    address[] queueStakerForNFT;
    //minimum block have to take to earn NFT
    uint256 public blockStakeForNFT;
    ...
    event NewNFTSlot(address user);
    event ClaimNFT(
        address indexed user,
        address indexed nftAddress,
        uint256 tokenId
    );
    event NewNftAddress(address nftAddress);
    event NewNftTokenIds(uint256[] nftIds);
    event NewMinStakeForNFT(uint256 minStakeForNFT);
    event NewBlockStakeForNFT(uint256 blockStakeForNFT);

    constructor(string memory poolName, string memory symbol)
        ERC20(poolName, symbol)
    {
        MSTATION_CASTLE_FACTORY = msg.sender;
    }
    ...
    function initialize(
        ...
        require(msg.sender == MSTATION_CASTLE_FACTORY, "Not factory");
        require(
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        // check user had nft reward, still exist slot for earn NFT, stake larger than min requirement, time enough for blockStakeForNFT
        if (
            minStakeForNFT > 0 &&
            user.amount >= minStakeForNFT &&
            (bonusEndBlock - block.number) >= blockStakeForNFT &&
            user.nftSlot == 0
        ) {
            user.nftStakingBlock = block.number;
            if (nftSlotTaken < nftIdRewards.length) {
                user.nftSlot = 1;
                nftSlotTaken++;
            } else {
                queueStakerForNFT.push(msg.sender);
            }
        }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        if (_amount > 0) {
            ...
            // check reward nft condition for keep or remove slot.
            if (
                user.nftSlot == 1 &&
                user.amount < minStakeForNFT &&
                (block.number - user.nftStakingBlock) < blockStakeForNFT
            ) {
                user.nftSlot = 0;
                if (queueStakerForNFT.length > 0) {
                    address nextUserAddress = queueStakerForNFT[0];
                    UserInfo storage nextUser = userInfo[nextUserAddress];
                    queueStakerForNFT[0] = queueStakerForNFT[
                        queueStakerForNFT.length - 1
                    ];
                    queueStakerForNFT.pop();
                    nextUser.nftSlot = 1;
                } else {
                    nftSlotTaken--;
                }
    ...
    function claimNFT() external nonReentrant {
        UserInfo storage user = userInfo[msg.sender];
        require(user.nftSlot == 1, "Address don't have NFT reward");
        require(
            (block.number - user.nftStakingBlock) >= blockStakeForNFT,
            "Staking has not enough time"
        );
        require(nftIdRewards.length > 0, "No NFT for reward");

        user.nftSlot = 2; // claimed
        IERC721 nft = getNftContract(nftAddress);
        uint256 nftTokenId = nftIdRewards[0];
        nft.safeTransferFrom(address(this), msg.sender, nftTokenId);
        nftIdRewards[0] = nftIdRewards[nftIdRewards.length - 1];
        nftIdRewards.pop();

        emit ClaimNFT(msg.sender, nftAddress, nftTokenId);
    }
    ...
    /**
     * @notice It allows the admin to update NFT address in standard erc721
     * @dev This function is only callable by owner.
     * @param _nftAddress: the new nft address
     */
    ...
    /**
     * @notice It allows the admin to update list token id of NFT reward
     * @dev This function is only callable by owner.
     * @param _nftIds: the new nft token ids
     */
    function updateNftTokenIds(uint256[] memory _nftIds) external onlyOwner {
        require(block.number < unStakingBlock, "Unstaking has started");
        nftIdRewards = _nftIds;
        emit NewNftTokenIds(_nftIds);
    }

    /**
     * @notice It allows the admin to update min stake amount for earn NFT
     * @dev This function is only callable by owner.
     * @param _minStakeForNFT: the new min stake amount
     */
    function updateMinStakeForNFT(uint256 _minStakeForNFT) external onlyOwner {
        require(block.number < unStakingBlock, "Unstaking has started");
        minStakeForNFT = _minStakeForNFT;
        emit NewMinStakeForNFT(_minStakeForNFT);
    }

    /**
     * @notice It allows the admin to update number of block have to stake to earn NFT
     * @dev This function is only callable by owner.
     * @param _blockStakeForNFT: the new min block stake
     */
    function updateBlockStakeForNFT(uint256 _blockStakeForNFT)
        external
        onlyOwner
    {
        require(block.number < unStakingBlock, "Unstaking has started");
        blockStakeForNFT = _blockStakeForNFT;
        emit NewBlockStakeForNFT(_blockStakeForNFT);
    }
    ...
    /*
     * @notice check reward NFT of user. 0: no reward, 1 a pending reward, 2 claimed reward.
     * @param _user: user address
     */
    function getPendingNftReward(address _user)
        external
        view
        returns (uint256)
    {
