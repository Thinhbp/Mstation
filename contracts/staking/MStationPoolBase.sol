    ...
    ERC20Upgradeable
{
    ...
    mapping(ERC20Upgradeable => uint256) public accTokenPerShare;
    ...
    mapping(ERC20Upgradeable => uint256) public rewardPerBlock;

    // The precision factor
    mapping(ERC20Upgradeable => uint256) public PRECISION_FACTOR;

    // The reward token
    ERC20Upgradeable[] public rewardTokens;

    // The staked token
    ERC20Upgradeable public stakedToken;
    ...
    struct UserInfo {
        ...
        mapping(ERC20Upgradeable => uint256) rewardDebt; // Reward debt
    }
    ...
    event NewRewardPerBlock(uint256 rewardPerBlock, ERC20Upgradeable token);
    ...
    event RemoveRewardToken(ERC20Upgradeable token);
    ...
    function initialize(string memory poolName, string memory symbol)
        public
        initializer
    {
        __Ownable_init();
        __ReentrancyGuard_init();
        __ERC20_init(poolName, symbol);
    ...
    function deposit(uint256 _amount) external nonReentrant {
        ...
        if (user.amount > 0) {
            ...
            for (uint256 i = 0; i < rewardTokens.length; i++) {
                ...
                if (pending > 0) {
                    ERC20Upgradeable(rewardTokens[i]).transfer(
                        address(msg.sender),
                        pending
                    );
                }
        ...
        if (_amount > 0) {
            ...
            ERC20Upgradeable(stakedToken).transferFrom(
                address(msg.sender),
                address(this),
                _amount
            );
    ...
    function safeERC20Transfer(
        ERC20Upgradeable _ERC20Upgradeable,
        address _to,
        uint256 _amount
    ) private {
        uint256 balance = _ERC20Upgradeable.balanceOf(address(this));
        if (_amount > balance) {
            _ERC20Upgradeable.transfer(_to, balance);
        } else {
            _ERC20Upgradeable.transfer(_to, _amount);
        }
    }
    ...
    function withdraw(uint256 _amount) external nonReentrant {
        ...
        for (uint256 i = 0; i < rewardTokens.length; i++) {
            pending = user
                ...
                .sub(user.rewardDebt[rewardTokens[i]]);
            if (pending > 0) {
                // ERC20Upgradeable(rewardTokens[i]).transfer(address(msg.sender), pending);
                safeERC20Transfer(
                    ERC20Upgradeable(rewardTokens[i]),
                    address(msg.sender),
                    pending
                );
            }
        ...
        if (_amount > 0) {
            ...
            ERC20Upgradeable(stakedToken).transfer(
                address(msg.sender),
                _amount
            );
            //_burn(address(msg.sender),_amount);
        ...
        returns (uint256)
    {
        ...
        if (
            ...
            ERC20Upgradeable(stakedToken).transfer(feeCollector, collectedAmt);
    ...
    function emergencyWithdraw() external nonReentrant {
        ...
        if (amountToTransfer > 0) {
            safeERC20Transfer(
                ERC20Upgradeable(stakedToken),
                address(msg.sender),
                amountToTransfer
            );
            // ERC20Upgradeable(stakedToken).transfer(address(msg.sender), amountToTransfer);
        }
        ...
        onlyOwner
    {
        ...
        ERC20Upgradeable(_tokenAddress).transfer(
            address(msg.sender),
            _tokenAmount
        );
    ...
    function pendingReward(address _user)
        external
        view
        returns (uint256[] memory, ERC20Upgradeable[] memory)
    {
    ...
    function pendingRewardByToken(address _user, ERC20Upgradeable _token)
        external
        view
        returns (uint256)
    {
     ...
     */
    function addRewardToken(ERC20Upgradeable _token, uint256 _rewardPerBlock)
        external
        onlyOwner
    {
     ...
     */
    function removeRewardToken(ERC20Upgradeable _token) external onlyOwner {
        require(address(_token) != address(0), "Must be a real token");
        require(address(_token) != address(this), "Must be a real token");
        ...
        require(foundToken, "Cannot find token");
        (bool success, ERC20Upgradeable[] memory newRewards) = removeElement(
            tokenIndex,
            rewardTokens
        );
     ...
     */
    function removeElement(uint256 _index, ERC20Upgradeable[] storage _array)
        internal
        returns (bool, ERC20Upgradeable[] memory)
    {
    ...
    function findElementPosition(
        ERC20Upgradeable _token,
        ERC20Upgradeable[] storage _array
    ...
    function getUserDebt(address _usr)
        external
        view
        returns (ERC20Upgradeable[] memory, uint256[] memory)
    {
    ...
    function getUserDebtByToken(address _usr, ERC20Upgradeable _token)
        external
        view
        returns (uint256)
    {
    ...
    function getAllRewardPerBlock(ERC20Upgradeable[] memory _tokens)
        external
        view
    ...
    function getAllAccTokenPerShared(ERC20Upgradeable[] memory _tokens)
        external
        view
        returns (uint256[] memory)
    {
    ...
    function getAllPreFactor(ERC20Upgradeable[] memory _tokens)
        external
        view
        returns (uint256[] memory)
    {
